...
handles.DefVar.method=8;
handles.tm.Text='Tracking Method (CARD)';
%[1:KLT+concer,2:KLT+surf,3:KTL+harris,4:KTL+FAst
...
if isfield(handles.DefVar,'flowRecommand')
...
if isequal(ReadVideoFileName,0) || isequal(Defaultdir,0) || isequal(ReadVideoFilterIndex,0)
    ...
    fullname=fullfile( Defaultdir,ReadVideoFileName);
...
switch obj.Tag
    case 'VideoPlaySlider'
        ...
        %          htableData{6}=[num2str(handles.xRate),'��'];
    case 'CurrentMin'
...
while hasFrame(hData)&&hData.CurrentTime<(hData.Duration-0.3)
    ...
    hData.CurrentTime=hData.CurrentTime+0.2;%%%%%%%%%%%%%%%%%%%%%%%����ʱ����
...
function series=get_series(flowMagnitude,threshM,picNum)%%plot_hht(ע��·����
series=[];
i=1;
...
if isequal(ReadVideoFileName,0) || isequal(Defaultdir,0) || isequal(ReadVideoFilterIndex,0)
    ...
    conf=open(fullname);
    conf.fullname=replace(conf.fullname,'E:\�ѹ�����ɸΰ�����\', 'E:\fusan\DONE\');
...
% handles    structure with handles and user data (see GUIDATA)
tic
if ~isempty(handles.DefVar.imRect) %hObject.Value==1&&
    [ShotingNum,ShotingFrameIdx]=findShotingNum( handles.DefVar.currentFrameNum,handles.DefVar.FrameNumsArray);
   ...
   method=handles.DefVar.method;
    p=gcp();
%[a,b]= track(stdimg,pos,CutPos,hData,cfn,bsfn,esfn,1,method);
if cfn==bsfn
    [fna1,rectsA1]= track(stdimg,pos,CutPos,hData,cfn,bsfn,esfn,2,method);
    fna2=[];
    rectsA2=zeros(1,4);
elseif esfn==bsfn
    [fna2,rectsA2]= track(stdimg,pos,CutPos,hData,cfn,bsfn,esfn,1,method);
   fna1=[];
    rectsA1=zeros(1,4);
else
        %[fna1,rectsA1]= track(stdimg,pos,CutPos,hData,cfn,bsfn,esfn,2,method);
        %fna2,rectsA2]= track(stdimg,pos,CutPos,hData,cfn,bsfn,esfn,1,method);
    for idx=1:2
        f(idx)=parfeval(p,@track,2,stdimg,pos,CutPos,hData,cfn,bsfn,esfn,idx,method);
    end
    ...
    set(handles.axes3,'YTick',[],'YTickLabel',[]);
     time=toc;
     if  size(a,1)<6
         handles.DefVar.fps=0;
     else
      handles.DefVar.fps=size(frameNumsArray,1)/time;
     end
    guidata(hObject,handles);
   
    %disp(size(frameNumsArray,1)/time);
end
...
%  handles.DefVar.imRect=hobject;���ø��ٿ���ʲô���
switch label
...
mirroPos=[];
if withInRect(pos,ErweiPos+[-10,-10,20,20])
    mirroPos(1)=-ErweiPos(1)+pos(1)+ZaoyingPos(1);
    mirroPos(2)=pos(2);
    mirroPos(3)=pos(3);
    mirroPos(4)=pos(4);
elseif withInRect(pos,ZaoyingPos+[-10,-10,20,20])
    mirroPos(1)=ErweiPos(1)+pos(1)-ZaoyingPos(1);
    mirroPos(2)=pos(2);
    mirroPos(3)=pos(3);
    mirroPos(4)=pos(4);
...
if hObject.Value==1
    ...
    if ableToAuto
        ...
        flowCommand=zeros(40,length(time_array),3)+192;%����ɫ ��ɫ ������ͣ
                %        figure;
                %        imshow(flowCommand);
        
                %        imshow(flowCommand);
                for wl=1:size(BofangTimeSeries,1)%��ɫ ������������
                ...
                for wl=1:size(ShuangfuTimeSeries,1) %��ɫ ����˫���仯��
                ...
                for wl=1:length(out_series)
                    for wll=1:size(out_series{wl},1) % ��ɫ ˫���Ƽ�
...
%        get(hObject,'Min') and get(hObject,'Max') to determine range of slider
handles=guidata(hObject);
set(handles.edit4, 'String',num2str(round(hObject.Value)));
guidata(hObject,handles);
...
%        str2double(get(hObject,'String')) returns contents of edit4 as a double
   handles=guidata(hObject);
    value=min(max(round(str2double(hObject.String)),1.0),400.0);
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
handles.DefVar.method=1;
handles.tm.Text=hObject.Text;
 guidata(hObject,handles);
...
sep=filesep;
tempath=strcat(pathstr,sep,name);
selpath = uigetdir(tempath);
if exist(selpath,'dir')
    if ~exist(strcat(selpath,'/',name,'_ROI'),'dir')
        mkdir(strcat(selpath,'/',name,'_ROI'));
    end
    for i =1:length(RectsArray)
        for j=1:length(RectsArray{i})
            rectnow=RectsArray{i}(j,:);
            imgnow=read(handles.DefVar.hData,FrameNumsArray{i}(j));
            imgnow=imcrop(imgnow,rectnow);
            imwrite(imgnow,strcat(selpath,'/',name,'_ROI/',j,'_',i,'.bmp'));
        end
    end
end
